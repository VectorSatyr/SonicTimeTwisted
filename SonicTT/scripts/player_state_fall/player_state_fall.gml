...
if (input_check(cLEFT))
{
	...
	if (xspeed > -speed_cap)
	{
		xspeed = max(xspeed - air_acceleration, -speed_cap);
	}
}
if (input_check(cRIGHT))
{
	...
	if (xspeed < speed_cap)
	{
		xspeed = min(xspeed + air_acceleration, speed_cap);
	}
}
...
if (landed)
{
	...
	case 3: // Knuckles
		if (glide_falling)
		{
			...
			{
				return (xspeed != 0) ? player_is_running() : 
					player_is_standing();
			}
			else
			{
	...
	// land normally
	return (xspeed != 0) ? player_is_running() : player_is_standing();
}
...
if (jump_action)
{
	...
	if (
		objProgram.special_future_current_level >= 7 and
		objGameData.rings[0] >= 50 and character_id == 1 and 
		not (superform or invincibility) and player_transform_input()
	)
	{
		return player_is_transforming();
	}
	else if (input_check_pressed(cACTION))
	{
		...
		else
		{
			...
			case 1: // Sonic
				...
				case 5:
					if (not underwater) return player_is_shield_flying();
					break;
...
// animate
if (animation == "rise" and yspeed >= 0)
{
	animation_new = "walk";
...
if (not spinning and image_angle != angle)
{
	image_angle = angle_wrap(
		image_angle + 2.8125 * sign(angle_difference(angle, image_angle))
	);
}